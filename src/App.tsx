...
const App: FC = () => {
  ...
  const [title, setTitle] = useState('Tendencias/Destacados');
  ...
  const searchPhotosHandler = (query: string) => {
    ...
    setTitle(`Buscar resultados de "${query}"`);
  }
  ...
  const showTrendingPhotos = () => {
    setLoading(true);
    setTitle('Tendencias/Destacados');
    setMode('trending');
    dispatch(getCuratedPhotos(1, () => setLoading(false), () => setLoading(false)));
  }
      ...
      error 
        ...
        : <Fragment>
          {mode === 'search' && <div className="is-flex is-justify-content-center pt-5">
            <button className="button is-link" onClick={showTrendingPhotos}>Regresar a los destacados</button>
          </div>}
          ...
          {photos.length > 0
            ...
            : <p className="has-text-centered">No hay resultados</p>
          }
          ...
          <div className="is-flex is-justify-content-center py-6">
            {((total_results > page * 10) || mode === 'trending') 
              && <button className="button is-primary is-large" onClick={loadMoreHandler} disabled={btnLoading}>
                  {!btnLoading ? 'Cargar Mas Fotos' : <div className="loading loading--small"></div>}    
                </button>
            }
          </div>
        </Fragment>
